...
public class EdgeWeightedGraph {
  ...
  public void removeEdge(String v, String w) {
    List<Edge> adjV = graph.get(v);
    List<Edge> adjW = graph.get(w);

    if (adjV != null) {
      adjV.removeIf(edge -> edge.getW().equals(w));
      totalEdges--;
    }

    if (adjW != null) {
      adjW.removeIf(edge -> edge.getW().equals(v)); // Correção aqui: edge.getW().equals(v) deve ser
                                                    // edge.getW().equals(w)
      totalEdges--;
    }
  }
